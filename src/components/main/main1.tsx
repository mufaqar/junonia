...
const Main1 = () => {
  ...
  useEffect(()=>{
     windowSize[0] <= 1080 ? setPost(PostMokeData.slice(1,3)) : setPost(PostMokeData.slice(1,5)) 
  })
  return (
    <>
          ...
          <div className="grid grid-cols-2 _grid h-full">
            {
               post.map((item:any, idx:number)=>{
                    return(
                      <MainPost post={item} className="text-sm md:text-lg "/>
                    )
               })
            }
        </div>
  ...
  return (
    ...
    >
      <div className="absolute top-0 right-0 p-4 md:p-10 flex flex-col justify-end left-0 bottom-0 bg-black/30">
        ...
        <Link href="#">
        <h2 className={`uppercase hover:underline mt-2 max-w-[480px] font-medium text-white ${className}`}>
          {post?.title}
        </h2>
        </Link>
